...
- one read-only resource for current session context and one static boundary resource
...
5. read the public alpha boundary resource
6. report current document state or read the current document resource
...
- "Read the `solidworks://alpha/public-boundary` resource."
...
- second public Resource MCP supported locally: yes
