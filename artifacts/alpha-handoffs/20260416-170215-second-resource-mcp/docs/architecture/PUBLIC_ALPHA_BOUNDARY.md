...
| Prompt | Implemented | Notes | Public alpha supported |
| --- | --- | --- | --- |
| `safe_modeling_session` | Yes | Guides the client toward the current safe workflow and stops clearly outside the boundary | Yes |

### Resources

| Resource | Implemented | Notes | Public alpha supported |
| --- | --- | --- | --- |
| `current_document_state` | Yes | Read-only current document snapshot at `solidworks://document/current-state` | Yes |
| `public_alpha_boundary` | Yes | Static read-only boundary summary at `solidworks://alpha/public-boundary` | Yes |
...
- use `safe_modeling_session` when prompt discovery is available and the session should stay aligned to the public alpha workflow
- use `public_alpha_boundary` when the host wants a static machine-friendly summary of the current public alpha boundary
- use `current_document_state` when the host wants read-only current session context
...
- `cut_extrude`
- `add_fillet`
- prompts that imply broader CAD capability than the current public tool surface
- resources that imply historical document browsing or open/reopen-document support beyond the current public pair
